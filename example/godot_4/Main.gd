...
func _ready():
	...
	Share.connect("on_error", Callable(self, "_on_error"))
	Share.connect("on_saved_to_gallery", Callable(self, "_on_saved_to_gallery"))
...
func _on_ShareImageBtn_pressed():
	...
	$Canvas/Center/VBox/Image.texture.get_image().save_png(image_path)
